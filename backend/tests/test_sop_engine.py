...
執行方式：
  cd c:\\Users\\alisa\\Downloads\\squirrel
...
def _nearest_snapshot_ts(target_ts: str) -> str:
    """
    回傳資料中 <= target_ts 的最新時間戳。
    （get_snapshot 要求 timestamp 必須在 available_timestamps() 中，
      但事件 timestamp 不一定是整點，因此做此轉換。）
    """
...
def run_case_a(incidents: dict):
    ...
    # 規格書驗證：ETE = 83.4 分鐘（RD_TPE_002飽和度1.00 + RD_TPE_004飽和度0.78）
...
def run_case_b(incidents: dict):
    ...
    # 驗證警力 police_needed：RD_TPE_007 有3個路口 → 3×2=6
    assert "6" in sop5.actions[0], (
        f"❌ 警力人數預期6人，actions: {sop5.actions}"
    )
    # ETE：Medium base=20 + 壅塞加罰
...
def run_case_c(incidents: dict):
    ...
    # 驗證 cascade_checks 含 affected_road 備註
